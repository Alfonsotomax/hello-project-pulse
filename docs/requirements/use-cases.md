...
- The course admin selects built-in templates for a team (hardcoded for the MVP; document creation is specified as UC-TPL-provision-documents)
...
- **4a. No matching rubrics are found:**
  ...
  - 4a2. The course admin either chooses to UC-RUB-create-rubric: Create a rubric, or terminates the use case, or returns to step 2 of the normal flow.

**Priority:** High
**Frequency of Use:** Occasional; mostly at course setup and around evaluation milestones.
**Business Rules:** BR-role-based-access — rubrics are scoped to the course admin's course.
...
**Related Use Cases:** UC-RUB-create-rubric: Create a rubric.
**Assumptions:**
**Open Issues:**
...
**Main Success Scenario:**
1. The course admin finds a list of rubrics through UC-RUB-find-rubrics: Find rubrics.
...
**Business Rules:** BR-role-based-access — a course admin may view only rubrics in her own course.
...
**Related Use Cases:** UC-RUB-find-rubrics: Find rubrics; UC-RUB-edit-rubric: Edit a rubric.
**Assumptions:**
**Open Issues:**
...
**Main Success Scenario:**
1. The course admin selects a rubric to edit (found through UC-RUB-find-rubrics: Find rubrics).
...
**Related Use Cases:** UC-RUB-find-rubrics: Find rubrics; UC-RUB-add-criterion: Add a criterion to a rubric; UC-RUB-remove-criterion: Remove a criterion from a rubric.
**Assumptions:**
**Open Issues:**
...
**Main Success Scenario:**
1. The course admin selects a rubric to delete (found through UC-RUB-find-rubrics: Find rubrics).
...
**Related Use Cases:** UC-RUB-find-rubrics: Find rubrics.
**Assumptions:**
**Open Issues:**
...
**Main Success Scenario:**
1. The course admin selects a rubric (found through UC-RUB-find-rubrics: Find rubrics).
2. The course admin selects a criterion to add (found through UC-RUB-find-criteria: Find criteria).
...
**Related Use Cases:** UC-RUB-remove-criterion: Remove a criterion from a rubric; UC-RUB-find-criteria: Find criteria; UC-RUB-create-criterion: Create a criterion.
**Assumptions:**
**Open Issues:**
...
**Related Use Cases:** UC-RUB-add-criterion: Add a criterion to a rubric.
**Assumptions:**
**Open Issues:**
...
**Main Success Scenario:**
1. The course admin selects a course section (found through UC-SEC-find-sections: Find course sections).
2. The course admin selects a rubric to assign (found through UC-RUB-find-rubrics: Find rubrics).
...
**Related Use Cases:** UC-SEC-find-sections: Find course sections; UC-SEC-view-section: View a course section; UC-RUB-find-rubrics: Find rubrics.
**Assumptions:**
**Open Issues:**
...
- **4a. No matching criteria are found:**
  ...
  - 4a2. The course admin either chooses to UC-RUB-create-criterion: Create a criterion, or terminates the use case, or returns to step 2 of the normal flow.

**Priority:** High
**Frequency of Use:** Occasional; mostly at course setup and rubric revision.
**Business Rules:** BR-role-based-access — criteria are scoped to the course admin's course.
...
**Main Success Scenario:**
1. The course admin finds a list of criteria through UC-RUB-find-criteria: Find criteria.
...
**Priority:** Medium
**Frequency of Use:** Occasional.
**Business Rules:** BR-role-based-access — a course admin may view only criteria in her own course.
...
**Related Use Cases:** UC-RUB-find-criteria: Find criteria; UC-RUB-edit-criterion: Edit a criterion.
**Assumptions:**
**Open Issues:**
...
**Main Success Scenario:**
1. The course admin selects a criterion to edit (found through UC-RUB-find-criteria: Find criteria).
...
**Related Use Cases:** UC-RUB-find-criteria: Find criteria.
**Assumptions:**
**Open Issues:**
...
**Main Success Scenario:**
1. The course admin selects a criterion to delete (found through UC-RUB-find-criteria: Find criteria).
2. The system asks the course admin to confirm the deletion.
3. The course admin confirms the deletion.
...
**Related Use Cases:** UC-RUB-find-criteria: Find criteria; UC-RUB-remove-criterion: Remove a criterion from a rubric.
**Assumptions:**
**Open Issues:**
...
- **4a. No matching course sections are found:**
  ...
  - 4a2. The course admin either chooses to UC-SEC-create-section: Create a course section or chooses to terminate the use case or chooses to return to step 2 of the normal flow.
...
2. The course admin finds a list of course sections through UC-SEC-find-sections: Find course sections.
...
- **5a. Rubric does not exist or the course admin wants to create a new rubric:**
  - 5a1. The course admin chooses to UC-RUB-create-rubric: Create a rubric.
...
2. The course admin views the details of this course section through UC-SEC-view-section: View a course section.
...
- **4a. No matching teams are found:**
  ...
  - 4a2. The user either chooses to UC-TEA-create-team: Create a team or chooses to terminate the use case or chooses to return to step 2 of the normal flow.
...
2. The user finds a list of teams through UC-TEA-find-teams: Find teams.
...
2. The course admin views the details of this team through UC-TEA-view-team: View a team.
...
2. The course admin views the details of the team through UC-TEA-view-team: View a team.
...
**Related Use Cases:** The course admin may then choose to UC-TEA-assign-students: Assign students to teams and UC-INS-assign-instructors: Assign instructors to teams.
**Assumptions:**
**Open Issues:**
...
- **4a. No matching students are found:**
  ...
  - 4a2. The user either chooses to UC-STU-invite-students: Invite students to join a course section or chooses to terminate the use case or chooses to return to step 2 of the normal flow.
...
2. The user finds a list of students through UC-STU-find-students: Find students.
...
2. The course admin views the details of this student through UC-STU-view-student: View a student.
...
2. The course admin or instructor views the details of this student through UC-STU-view-student: View a student.
...
**Related Use Cases:** UC-STU-delete-student: Delete a student; UC-STU-reactivate-student: Reactivate a student
**Assumptions:**
**Open Issues:**
...
- **4a. No matching instructors are found:**
  ...
  - 4a2. The course admin either chooses to UC-INS-invite-instructors: Invite instructors to register an account or chooses to terminate the use case or chooses to return to step 2 of the normal flow.
...
2. The course admin finds a list of instructors through UC-INS-find-instructors: Find instructors.
...
2. The course admin views the details of this instructor through UC-INS-view-instructor: View an instructor.
...
2. The instructor views the details of this student through UC-STU-view-student: View a student.
...
7. The system notes that a submitted peer evaluation remains editable while its submission window is open; the student may re-submit it to update it until the window closes (BR-evaluation-editable-until-close, BR-evaluation-submission-window).
...
Report generating algorithm: For each individual criterion score (e.g., Quality of work), the system shall consider the scores provided by all teammates and compute an average. For the overall grade, see the algorithm in UC-EVA-section-evaluation-report: Generate a peer evaluation report of the entire course section.

**Related Use Cases:** UC-EVA-section-evaluation-report:Generate a peer evaluation report of the entire course section
**Assumptions:**
**Open Issues:**
...
Report generating algorithm: Refer to the algorithm defined in UC-EVA-section-evaluation-report: Generate a peer evaluation report of the entire course section.

Details of a peer evaluation: The instructor may choose to see more details of one student's peer evaluation. Refer to the algorithm defined in UC-EVA-section-evaluation-report: Generate a peer evaluation report of the entire course section.
...
**Business Rules:** BR-document-creation — Only a course admin may create a team's requirement documents, and each document type is created at most once per team (an existing document cannot be regenerated or overwritten).
**Associated Information:**
...
**Related Use Cases:** UC-GLO-view-glossary: View the Project Glossary; UC-DOC-view-document: View a section-based requirement document; UC-DOC-view-use-cases-document: View the Use Cases document.
**Assumptions:**
**Open Issues:**
...
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control).
**Associated Information:**
...
- **4a. No matching glossary terms are found**
  ...
  - 4a2. The student either chooses to UC-GLO-create-term: create a glossary term or chooses to terminate the use case or chooses to return to step 2 of the normal flow.
...
- UC-GLO-view-term: View a glossary term
- UC-GLO-create-term: Create a glossary term
- UC-GLO-edit-term-definition: Change a glossary term definition
- UC-GLO-rename-term: Rename a glossary term
- UC-GLO-delete-term: Delete a glossary term
...
2. The student finds a list of glossary terms through UC-GLO-find-terms: Find glossary terms.
...
**Related Use Cases:** UC-GLO-find-terms: Find glossary terms
**Assumptions:**
**Open Issues:**
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-glossary-term-unique (a glossary term name is unique within the team's Glossary).
**Associated Information:**
...
**Related Use Cases:** The student may first choose to UC-GLO-find-terms: Find glossary terms, but cannot find any, then decide to create one.
**Assumptions:**
**Open Issues:**
...
2. The student views the definition of this glossary term through UC-GLO-view-term: View a glossary term.
...
**Related Use Cases:** The student may first choose to UC-GLO-view-term: View a glossary term, then decide to change one.
**Assumptions:**
**Open Issues:**
...
**Related Use Cases:** The student may first choose to UC-GLO-view-term: View a glossary term, then decide to rename one.
**Assumptions:**
**Open Issues:**
...
2. The student views the details of this glossary term through UC-GLO-view-term: View a glossary term.
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-deletion-integrity (a referenced term cannot be deleted; deletion is logical).
**Associated Information:**
...
**Related Use Cases:** The student may first choose to UC-GLO-view-term: View a glossary term, then decide to delete one.
**Assumptions:**
**Open Issues:**
...
2. The student views the document sections of the document through UC-DOC-view-document: View a section-based requirement document.
...
Details:
- The set of document sections, their order, and the rules for each section of a section-based document (Vision and Scope, SRS) are defined by that document's built-in Template (see UC-TPL-provision-documents: Create team documents from built-in templates). This use case edits one document section at a time within that template-defined structure.

Notification: None — routine authoring changes are not notified (FR-NOT-suppress-routine); saved changes propagate to connected teammates in real time (UC-COL-collaborative-edit).

The student shall be able to cancel the use case at any time prior to submitting it.

**Related Use Cases:** The student may first choose to UC-DOC-view-document: View a section-based requirement document, then decide to edit one.
**Assumptions:**
**Open Issues:**
...
3. The student may choose to view the details of one specific use case through UC-DOC-view-use-case: View a use case.
4. Use case ends.
...
**Related Use Cases:** UC-DOC-view-use-case: View a use case; UC-DOC-create-use-case: Create a use case.
**Assumptions:**
**Open Issues:**
...
2. The student finds a list of use cases through UC-DOC-view-use-cases-document: View the Use Cases document.
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-use-case-name-unique (a use case name is unique within the team's Use Cases document).
**Associated Information:**
...
**Related Use Cases:** The student may first choose to UC-DOC-view-use-cases-document: View the Use Cases document but cannot find the use case, then decide to create one.
**Assumptions:**
**Open Issues:**
...
2. The student views the details of this use case through UC-DOC-view-use-case: View a use case.
...
- **4a. No matching requirement artifacts are found**
  ...
  - 4a2. The student either chooses to UC-ART-create-artifact: create a requirement artifact or chooses to terminate the use case or chooses to return to step 2 of the normal flow.
...
| Search property name | Data type | Options | Validation rule | Security/access concerns | Reference to glossary |
| ---- | ---- | ---- | ---- | ---- | ---- |
| Keyword | String (free text) | — | Optional; trimmed; matched case-insensitively as a fuzzy match across the artifact's content and Artifact Key | Team-scoped (BR-team-scoped-access, BR-role-based-access) | Requirement Artifact |
| Artifact key | String | — | Optional; matched case-insensitively (e.g., FR-1, UC-3) | Team-scoped (BR-team-scoped-access, BR-role-based-access) | Artifact Key |
| Artifact content | String (free text) | — | Optional; fuzzy match against the artifact's body content | Team-scoped (BR-team-scoped-access, BR-role-based-access) | Requirement Artifact |
| Type | Enumeration | The artifact types defined by the Artifact Type glossary term | Optional; must be one of the defined artifact types | Team-scoped (BR-team-scoped-access, BR-role-based-access) | Artifact Type |
...
2. The student finds a list of requirement artifacts through UC-ART-find-artifacts: Find requirement artifacts.
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-artifact-key-unique (unique, stable artifact key). The document section the artifact is created in determines its placement and its artifact type.
**Associated Information:**
...
**Related Use Cases:** The student may first choose to UC-ART-find-artifacts: Find requirement artifacts but cannot find any, then decide to create one.
**Assumptions:**
**Open Issues:**
...
2. The student views the details of this requirement artifact through UC-ART-view-artifact: View a requirement artifact.
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-deletion-integrity (a referenced artifact cannot be deleted; deletion is logical).
**Associated Information:**
...
**Related Use Cases:** The student may first choose to UC-ART-view-artifact: View a requirement artifact, then decide to delete one.
**Assumptions:**
**Open Issues:**
...
**Related Use Cases:** UC-LNK-trace-requirement: Trace a requirement across levels — for an ordered, end-to-end path along abstraction levels rather than free local exploration.
**Assumptions:**
**Open Issues:**
...
2. The student finds a list of links through UC-LNK-view-links: View requirement artifact links.
...
**Related Use Cases:** UC-LNK-view-links: View requirement artifact links; UC-LNK-trace-requirement: Trace a requirement across levels.
**Assumptions:**
**Open Issues:**
...
2. The student views the details of one requirement artifact through UC-ART-view-artifact: View a requirement artifact.
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-link-constraints (link uniqueness, no self-linking, allowed link types only).
**Associated Information:**
...
The system validates the selected (source type, link type, target type) combination against the link-type compatibility matrix defined under BR-link-constraints in [business-rules.md](business-rules.md) (step 6; extension 6a handles a violation).
...
2. The student views the details of this link through UC-LNK-view-link: View a requirement artifact link.
...
A changed target artifact or link type is validated against the link-type compatibility matrix defined under BR-link-constraints in [business-rules.md](business-rules.md) (step 6; extension 6a handles a violation).
...
2. The student finds a list of high-level requirement artifacts through UC-ART-find-artifacts: Find requirement artifacts, e.g., business objectives.
...
**Related Use Cases:** UC-LNK-view-links: View requirement artifact links — free, any-direction, any-depth exploration of an artifact's links. This use case instead follows an ordered, goal-driven path along abstraction levels (e.g., Objective → Feature → use case → functional requirement). Difference: UC-LNK-view-links is free exploration; UC-LNK-trace-requirement is an ordered end-to-end path.
**Assumptions:**
**Open Issues:**
...
- **6a. The student corrects an issue and re-runs validation**
  - 6a1. From the highlighted location, the student edits the affected document section or use case to resolve the finding (UC-DOC-edit-document: Edit a document section, or UC-DOC-edit-use-case: Edit a use case).
...
**Main Success Scenario:**
1. The student opens a requirement document through UC-DOC-view-document: View a section-based requirement document (or UC-DOC-view-use-cases-document: View the Use Cases document).
...
4. The student edits a document section through UC-DOC-edit-document: Edit a section-based requirement document, or a use case through UC-DOC-edit-use-case: Edit a use case.
5. As the student acquires a lock and saves changes (within UC-DOC-edit-document / UC-DOC-edit-use-case), the system broadcasts the updated lock state and the saved content to all connected team members in real time.
...
- **4a. The student attempts to edit a document section or use case that is locked by a teammate**
  - 4a1. UC-DOC-edit-document / UC-DOC-edit-use-case handle this: the system does not grant the lock, and the student chooses a different target or waits.
...
**Priority:** High
**Frequency of Use:** Very frequent: multiple times per student per working session; peaks near deadlines.
**Business Rules:** BR-edit-lock-required, BR-collab-no-overwrite — Editing and locking of individual document sections and use cases are governed by UC-DOC-edit-document and UC-DOC-edit-use-case. The system shall ensure that real-time updates do not overwrite or corrupt content saved by other collaborators.
**Associated Information:**
...
**Related Use Cases:** UC-DOC-edit-document: Edit a section-based requirement document; UC-DOC-edit-use-case: Edit a use case; UC-DOC-view-document: View a section-based requirement document; UC-DOC-view-use-cases-document: View the Use Cases document.
**Assumptions:**
**Open Issues:**
...
**Preconditions:**
- PRE-1. The student is logged into the system.
- PRE-2. The student is a member of the team that owns the document or artifact (or, for review feedback, the instructor assigned to the course section — see UC-REV-review-documents).
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-comment-access (comments may be added by team students and the course section's instructor, regardless of the document's review-lock state).
**Associated Information:**

Details: Comments may be attached to document, document sections, and artifacts.

Notification: None — per the project's notification policy (FR-NOT-suppress-routine), creating a comment does not raise a notification; the comment is visible in the document, document section, or artifact view where it is attached, and appears to connected teammates in real time (UC-COL-collaborative-edit).
...
**Preconditions:**
- PRE-1. The student is logged into the system.
- PRE-2. The student is a member of the team that owns the comment (or, for review feedback, the instructor assigned to the course section — see UC-REV-review-documents).
...
2. The student finds a list of comments through UC-DOC-view-document: View a section-based requirement document or UC-ART-view-artifact: View a requirement artifact.
...
**Priority:** High
**Frequency of Use:** Varies by team and project phase; most frequent during active authoring and near deadlines.
**Business Rules:** BR-team-scoped-access, BR-role-based-access (team-scoped access control); BR-comment-access (comments may be resolved by team students and the course section's instructor, regardless of the document's review-lock state).
**Associated Information:**
**Related Use Cases:**
**Assumptions:**
**Open Issues:**
...
3. The instructor reads the documents and adds feedback through UC-COL-add-comment: Comment on a requirement document.
...
3. The instructor edits the teaching context content (e.g., standards, common-mistake guidance, and thinking order). Per-assistant behavior is configured separately through UC-CFG-configure-assistant-instructions.
...
**Business Rules:** BR-section-config-access — Only an instructor assigned to the course section may view or edit its teaching context. The teaching context is the context the course section's assistants draw on.
**Associated Information:**
...
- Realized by FR-AI-teaching-context.

**Related Use Cases:** UC-AI-elicit-requirements: Elicit requirements with the elicitation assistant; UC-AI-critique: Request a critique from the critique assistant; UC-AI-tutor: Ask an assistant to explain a concept (Tutor Mode); UC-AI-practice-interview: Practice a client interview with a role-playing client assistant; UC-CFG-toggle-assistants: Enable or disable AI assistants for a course section; UC-CFG-configure-assistant-instructions: Configure the assistant instructions for a course section.
**Assumptions:**
**Open Issues:**
...
**Description:** The instructor wants to enable or disable AI assistants — individually, per assistant — for the students in a course section she teaches, so that she controls whether and when each kind of AI help is permitted (for example, disabling the drafting assistant during a graded elicitation exercise while leaving the tutor assistant on). This setting governs the availability of the student-facing AI use cases (UC-AI-elicit-requirements, UC-AI-practice-interview, UC-AI-structure-notes, UC-AI-critique, UC-AI-tutor, UC-AI-draft-skeleton, UC-AI-consult-project-assistant, UC-AI-whole-project-review) for that course section's teams.
...
**Business Rules:** BR-section-config-access, BR-assistant-enablement — Only an instructor assigned to the course section may change its AI assistant settings. Assistants may be enabled or disabled individually. When an assistant is disabled, its corresponding student-facing use case is unavailable for that course section's students. The drafting assistant (UC-AI-draft-skeleton) shall be disabled by default.
**Associated Information:**
- Default state: the drafting assistant is off; other assistants' defaults are set at course setup.
- Realized by FR-AI-enablement.

**Related Use Cases:** UC-AI-elicit-requirements: Elicit requirements with the elicitation assistant; UC-AI-critique: Request a critique from the critique assistant; UC-AI-draft-skeleton: Generate a draft requirement skeleton with an assistant; UC-AI-tutor: Ask an assistant to explain a concept (Tutor Mode); UC-AI-practice-interview: Practice a client interview with a role-playing client assistant; UC-AI-structure-notes: Turn meeting notes into structured requirements; UC-CFG-configure-teaching-context: Configure the teaching context for a course section.
**Assumptions:**
**Open Issues:**
...
**Priority:** High
**Frequency of Use:** Occasional; at course setup and periodic refinement.
**Business Rules:** BR-section-config-access — Only an instructor assigned to the course section may view or edit its assistant instructions. Assistant instructions are authored per assistant and govern assistant behavior, while the teaching context governs the standards the assistant enforces.
**Associated Information:**
- Assistant instructions are the per-assistant behavioral layer; the teaching context (UC-CFG-configure-teaching-context) is the shared standards layer the instructions reference.
- Applies to every assistant in the course section.
- Realized by FR-AI-assistant-instructions.

**Related Use Cases:** UC-CFG-configure-teaching-context: Configure the teaching context for a course section; UC-CFG-toggle-assistants: Enable or disable AI assistants for a course section.
**Assumptions:**
**Open Issues:**
...
**Description:** The instructor wants to author and maintain the cross-document review criteria for a course section she teaches — the criteria the critique assistant applies when a student requests a whole-project review (UC-AI-whole-project-review) of the team's entire set of requirement documents. The criteria define the dimensions and standards the whole-project review checks across documents (for example: every feature has a downstream use case and functional requirement; every cited glossary term, business rule, and FR resolves; scope statements agree across documents; no conflicts between documents). Whereas the teaching context (UC-CFG-configure-teaching-context) supplies the course-section-wide, per-requirement quality standards and assistant instructions (UC-CFG-configure-assistant-instructions) supply per-assistant behavior, the cross-document review criteria define what the whole-project review evaluates across the document set. They must be defined before the whole-project review is available to the course section's students. A course admin, who is also an instructor of her course, may perform this use case.
...
- POST-2. The critique assistant uses the updated criteria on subsequent whole-project reviews (UC-AI-whole-project-review).
...
**Business Rules:** BR-source-material-import — Only a member of the team that owns the project may import its project source material. Imported materials are project inputs, not authored requirement content, and are not themselves graded as requirements.
**Associated Information:**
...
8. Once the client's input is verified, the student returns to the system and drafts the content herself through UC-DOC-edit-document: Edit a section-based requirement document (or UC-DOC-edit-use-case: Edit a use case), optionally translating her verified notes into candidate requirements through UC-AI-structure-notes: Turn meeting notes into structured requirements.
9. Use case ends.
...
- **6a. The student asks the assistant to draft the requirements from the answers**
  - 6a1. Per the assistant's configured Socratic behavior, the system declines to author finished requirements and instead helps the student structure her own draft (optionally via UC-AI-structure-notes), returning to step 8.
...
**Business Rules:** BR-assistant-enablement, BR-assistant-socratic, BR-authored-prevails — The elicitation assistant shall be Socratic — it shall coach the student's questioning, follow-up, and verification rather than author requirements for her. The assistant must be enabled for the course section (UC-CFG-toggle-assistants). The system shall not author or modify student content with assistant-generated text without explicit confirmation. Real clients are not participants in the system. The project source material is a kickoff input that may become stale; where it conflicts with later authored-and-verified requirements, the authored content prevails, and the assistant shall not treat the imported pitch materials as authoritative over the team's current, verified requirements. The student may exclude the project source material from the assistant's grounding for a session, directing the assistant to rely only on the current drafted requirements.
**Associated Information:**
...
- Real clients are deliberately kept out of the system; the client discussion happens off-platform. The student can rehearse it against a simulated client through UC-AI-practice-interview.
- The assistant grounds its gap analysis in the imported project source material (UC-AI-import-source-material) when available and not excluded for the session, and draws on the teaching context configured for the course section (UC-CFG-configure-teaching-context).
...
- PRE-1. The student is logged into the system.
- PRE-2. The student is a member of the team.
- PRE-3. The client role-play assistant is enabled for the student's course section (see UC-CFG-toggle-assistants).
...
**Business Rules:** BR-assistant-enablement, BR-assistant-socratic — The client role-play assistant shall stay in the non-technical client persona and shall not author requirements for the student. The assistant must be enabled for the course section (UC-CFG-toggle-assistants).
**Associated Information:**
...
- PRE-3. The structuring assistant is enabled for the student's course section (see UC-CFG-toggle-assistants).
...
5. The student reviews and accepts or rejects each candidate through UC-AI-review-proposal: Review and accept or reject an assistant proposal.
6. For each accepted candidate, the system applies it to its authoring destination as a draft the student authors — which requires her to hold that authoring destination's lock (a document section via UC-DOC-edit-document, or a use case via UC-DOC-edit-use-case) — and she refines and completes it through UC-DOC-create-use-case: Create a use case or UC-ART-create-artifact: Create a requirement artifact.
7. Use case ends.
...
- **4a. The notes contain nothing the assistant can structure (e.g., too sparse or off-topic)**
  - 4a1. The system tells the student that no candidates were produced and suggests adding detail or eliciting more from the client (UC-AI-elicit-requirements); the use case ends.
...
**Business Rules:** BR-assistant-socratic, BR-explicit-acceptance — Candidate requirements shall be applied only through the explicit acceptance loop of UC-AI-review-proposal (no "accept all"). The assistant must be enabled for the course section (UC-CFG-toggle-assistants). Each candidate shall show the note it derives from as rationale. An accepted candidate shall be applied to its authoring destination — a document section (UC-DOC-edit-document) or a use case (UC-DOC-edit-use-case) — and is subject to that authoring destination's lock; a candidate whose authoring destination is locked by another team member shall not be applied until the lock is released.
**Associated Information:**
...
**Description:** The student wants the critique assistant to evaluate an authored document section or use case for clarity, ambiguity, consistency, completeness, and testability, and to explain why each issue weakens the requirements — so that the student can improve it herself. The critique is qualitative, rationale-rich feedback that complements, and is distinct from, the deterministic ReqLint validation in UC-VAL-run-validation. The assistant's rationale is itself the lesson.

**Preconditions:**
- PRE-1. The student is logged into the system.
- PRE-2. The student is a member of the team that owns the requirement document.
- PRE-3. The critique assistant is enabled for the student's course section (see UC-CFG-toggle-assistants).
...
6. The student reviews a finding, navigates to the location, and revises the document section or use case herself through UC-DOC-edit-document: Edit a section-based requirement document (or UC-DOC-edit-use-case: Edit a use case).
7. Use case ends.
...
- **4a. The critique assistant proposes a concrete rewrite for a finding**
  - 4a1. The student reviews and accepts or rejects the proposed rewrite through UC-AI-review-proposal: Review and accept or reject an assistant proposal.

**Priority:** High
**Frequency of Use:** Frequent; typically after drafting and before submission.
**Business Rules:** BR-assistant-socratic, BR-explicit-acceptance — Every critique finding shall include an instructive rationale. The critique assistant must be enabled for the course section (UC-CFG-toggle-assistants). The assistant shall not modify student-authored content without explicit confirmation (UC-AI-review-proposal). Critique is advisory and distinct from ReqLint validation (UC-VAL-run-validation).
**Associated Information:**
...
- PRE-1. The student is logged into the system.
- PRE-2. The student is a member of the team.
- PRE-3. The tutor assistant is enabled for the student's course section (see UC-CFG-toggle-assistants).
...
- PRE-1. The student is logged into the system.
- PRE-2. The student is a member of the team that owns the content.
- PRE-3. The drafting assistant is enabled for the student's course section (see UC-CFG-toggle-assistants). It is disabled by default in course use.
...
5. The student reviews and accepts or rejects the proposal through UC-AI-review-proposal: Review and accept or reject an assistant proposal.
6. The accepted skeleton is applied as a draft on acceptance in step 5; the student then refines and completes it herself, authoring it out through UC-DOC-create-use-case: Create a use case (or UC-DOC-edit-use-case / UC-ART-create-artifact).
7. Use case ends.
...
4. On acceptance, the system applies the change to its authoring destination as authored content (recorded as the student's edit), which requires the student to hold that authoring destination's lock (a document section via UC-DOC-edit-document, or a use case via UC-DOC-edit-use-case). On rejection, the system discards the change.
...
- **2a. The student requests an explanation of a proposed change**
  - 2a1. The student asks the tutor assistant to explain through UC-AI-tutor: Ask an assistant to explain a concept, then returns to step 2.
...
5. The student follows a recommended action, and the system routes her into the corresponding use case (for example, UC-AI-elicit-requirements for elicitation or UC-AI-critique for a critique).
...
- **4a. The student asks the assistant to author or change requirement content**
  - 4a1. Per its Socratic configuration, the system declines to author content and instead routes the student to the appropriate authoring use case (for example, UC-DOC-edit-document or UC-DOC-edit-use-case) or specialized assistant.
...
**Primary Actor:** student
**Secondary Actors:** Critique assistant (realized via the LLM Service)
**Trigger:** The student indicates to request a whole-project review of the team's requirement documents (via a "Review whole project" action, or routed in from the project assistant in UC-AI-consult-project-assistant).
**Description:** The student wants the critique assistant to review the team's entire set of requirement documents together — not one selected document section or use case — so that she can find cross-document problems that are invisible when reviewing one item at a time: coverage gaps, inconsistencies, conflicts, and broken traceability across the documents. The review is qualitative, rationale-rich feedback that complements both the deterministic ReqLint validation (UC-VAL-run-validation) and the per-destination critique (UC-AI-critique). It is governed by the cross-document review criteria the instructor configures for the course section (UC-CFG-configure-review-criteria); the assistant's rationale is itself the lesson.

**Preconditions:**
- PRE-1. The student is logged into the system.
- PRE-2. The student is a member of the team that owns the documents.
- PRE-3. The critique assistant is enabled for the student's course section (see UC-CFG-toggle-assistants).
- PRE-4. The cross-document review criteria are defined for the student's course section (see UC-CFG-configure-review-criteria).
...
2. The system gathers the content of all the team's requirement documents, the relevant requirements-graph context for traceability, the course section's cross-document review criteria (UC-CFG-configure-review-criteria), and the teaching context (UC-CFG-configure-teaching-context).
...
- **1a. The cross-document review criteria are not defined for the course section**
  - 1a1. The system informs the student that the whole-project review is unavailable until her instructor defines the criteria (UC-CFG-configure-review-criteria) and terminates the use case.
- **3a. The critique assistant is disabled for the course section**
  - 3a1. The system informs the student that the critique assistant is unavailable and terminates the use case.
- **3b. The LLM Service is unavailable or times out**
  - 3b1. The system informs the student that AI assistance is temporarily unavailable and that the rest of RAM continues to operate, and terminates the use case.
